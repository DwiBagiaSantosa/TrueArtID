...
class Classifier (assetManager: AssetManager, modelPath: String, labelPath: String, inputSize: Int) {
    ...
    private fun convertBitmapToByteBuffer(bitmap: Bitmap): ByteBuffer {
        ...
        bitmap.getPixels(intValues, 0, bitmap.width, 0, 0, bitmap.width, bitmap.width)
        var pixel = 0
    ...
    private fun getSortedResult(labelProbArray: Array<FloatArray>): List<Classifier.Recognition> {
        ...
        val pq = PriorityQueue(MAX_RESULTS, Comparator<Recognition> {
        ...
        Log.d("Classifier", "pq_size:(%d)".format(pq.size))
